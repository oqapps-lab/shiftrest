...
- **Rebuild** ✅ TestFlight **build 1001** uploaded to App Store Connect 2026-05-31 (processing). Path: local `eas build --local` → manual fixes along the way:
  1. Caught + fixed a REAL bug the build surfaced: `expo-image-picker` was hand-pinned to wrong `17.x`; SDK 55 needs `~55.0.18` (unified versioning). 17.x used removed expo-modules-core Swift symbols (`EXFatal`) → ARCHIVE FAILED. Fixed `e500bc7`. (tsc/jest/metro all passed — only native build caught it.)
  2. macOS 26 export bug (`exportArchive Copy failed`): homebrew rsync 3.4.3 shadows Apple openrsync. Fix = lead PATH with `/usr/bin`. First IPA via manual exportArchive; build #3 ran the whole pipeline rsync-safe end-to-end.
  3. Apple rejected build 1000 (duplicate); bumped to 1001 (`b754f2c`).
- **Remaining (Apple-side / async):** wait for Apple processing to finish, then confirm build 1001 attaches to the Internal QA beta group so it's testable. Owner tests on device.
- **Live sim UI verify** of B3a/B3b stays UNVERIFIED on the dev sim this session (Expo Go bundle-download timed out / sim env); the TestFlight build is the real device verification.
